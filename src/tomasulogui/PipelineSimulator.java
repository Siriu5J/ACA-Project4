...
//      branchUnit.squashAll();
    ...
    public void updateCDB() {
        ...
        // Always prioritize on divider
        if (divider.isRequestWriteback()) {
            cdb.setDataTag(divider.getDestTag());
            cdb.setDataValue(divider.getResult());
            cdb.setDataValid(true);
            divider.setCanWriteback();
        }
        // Then do multiplier
        else if (multiplier.isRequestWriteback()) {
            cdb.setDataTag(multiplier.getDestTag());
            cdb.setDataValue(multiplier.getResult());
            cdb.setDataValid(true);
            multiplier.setCanWriteback();
        }
        // Then do ALU
        else if (alu.isRequestWriteback()) {
            cdb.setDataTag(alu.getDestTag());
            cdb.setDataValue(alu.getResult());
            cdb.setDataValid(true);
            alu.setCanWriteback();
        }
        // Finally, do load
